...
function brc(name) {
  return path.resolve(
    __dirname,
    `node_modules/buildo-react-components/src/${name}/${name}.tsx`
  );
}
    ...
    },
    {
      ...
      components: brcComponents
        .concat([
          path.resolve(
            __dirname,
            "node_modules/react-autosize-textarea/src/TextareaAutosize.tsx"
          ),
          path.resolve(
            __dirname,
            "node_modules/react-cookie-banner/src/CookieBanner.tsx"
          ),
          path.resolve(
            __dirname,
            "node_modules/react-flexview/src/FlexView.tsx"
          )
        ])
        .sort((a, b) =>
          a
  ...
  getComponentPathLine(componentPath) {
    ...
    switch (name) {
      case "TextareaAutosize":
        return `import TextareaAutosize from 'react-autosize-textarea'`;
      case "CookieBanner":
        return `import CookieBanner from 'react-cookie-banner'`;
      case "FlexView":
        return `import FlexView from 'react-flexview'`;
      default:
        return `import ${name} from 'buildo-react-components/lib/${name}'`;
    }
  },
